...
- [x] Ensure the timer math still displays correctly in both modes.
- [x] Keep the cumulative slide timer visible in the lower-right corner in presenter mode (chrome-hidden), not just in edit mode. Currently `body.chrome-hidden` hides `#slide-time` along with the nav chrome, so the presenter loses their clock exactly when they need it most.
